/*
  WiFi klient, kobler seg til skule.sormo.no/honsehus.php
  Port 1165
...
#define knapp_apne  35
#define knapp_lukk  34
#define knapp_stopp  33
...
void callback(char* topic, byte* payload, unsigned int length) {
  ...
  if   (status_var != "Deaktivert")
  {
    ...
    else if  (topicstr == "fart1")
    {
      ...
      client.publish("honsegard/fart3", charBuf);
    ...
    else if  (str.substring(0, 5) == "stopp")
    {
      status_var = "Stoppet";
      stopp();
    }
  ...
  if  (str.substring(0, 5) == "deact")
    deaktiver();
  else if  (str.substring(0, 5) == "activ")
    aktiver();
}
...
boolean reconnect() {
  ...
  while (!client.connected() && (teller++ < 4) && (!connected1)) {
    ...
    if (client.connect(clientId.c_str()))
    {
      ...
      str1.toCharArray(charBuf, 20);
      client.publish("honsegard/timeout", charBuf);
      ...
      if (EEPROM.read(54) == 0)
        client.publish("honsegard/apen_mode", "false");
      else if (EEPROM.read(55) == 0)
        client.publish("honsegard/lukket_mode", "false");
      else
        client.publish("honsegard/lukket_mode", "true");

      // ... and resubscribe
      client.subscribe("honsegard/#");
    }
  };
    if (teller > 4)
  {
...
boolean rc()
{
  ...
  while (!client.connected() && (count < 9) && (!connected1)) {
    ...
    if (client.connect(clientId.c_str())) {
      ...
      display.println();
      // Once connected, publish an announcement...
      client.publish("honsegard", "hello world");
      // ... and resubscribe
      client.subscribe("honsegard/#");
...
void setup()
{
  ...
  pinMode(bryter_apen, INPUT); //LOW: Lukket
  ...
  if (bryter_lukket == port_lukket) pos = timegrense;
  ...
  {
    client.publish("honsegard/bryter_lukket", "true");
  ...
  {
    client.publish("honsegard/bryter_lukket", "false");
    client.publish("honsegard/led", "deaktiver");
  };

  if (digitalRead(bryter_apen) == port_apen)
  {
    client.publish("honsegard/bryter_apen", "true");
    display.clearDisplay();
    display.setTextSize(3);
    display.setTextColor(WHITE);
    ...
    client.publish("honsegard/bryter_apen", "false");

  if ((digitalRead(bryter_lukket) != port_lukket) && (digitalRead(bryter_apen) != port_apen))
  {
    aktiver();
  }
...
void loop() {
  ...
  else
  {
    ...
    if (( digitalRead(bryter_lukket) == port_lukket) ||  ( digitalRead(bryter_apen) == port_apen))
    {
      Serial.println("brytere sluttet " + String( digitalRead(bryter_lukket) == port_lukket) + " : " + String ( digitalRead(bryter_apen) == port_apen));
      published1 = 0;
      if (pubmod == 1) {
        published = false;
       };
      if (( digitalRead(bryter_lukket) == port_lukket) &&  ( digitalRead(bryter_apen) == port_apen))
      {
        ...
        client.publish("honsegard", "Begge portposisjonene er meldt aktive");
      ...
      else
      {
        if (!published)
        {
          ...
          else if (digitalRead(bryter_apen) == port_apen)
          {
            if (pubmod != 3) {
              published = true;
              prevmode = pubmod;
    ...
    else  // her er ingen av porten sluttet.
    {
      ...
      {
        if (pubmod != 4) {
           client.publish("honsegard/led", "disabled");
          if (prevmode == 2) client.publish("honsegard/bryter_lukket", "false");
          if (prevmode == 3) client.publish("honsegard/bryter_apen", "false");
          pubmod1 = 4;
          published1 = true;
        }
      }
...
void vis_display()
{
  ...
  display.setTextColor(WHITE);
  display.setCursor(0, 0);
  display.println("WiFi SSID: dlink");
...
void lukk(String modus) {
  ...
  if (digitalRead(bryter_lukket) == port_lukket)
  {
    ...
    client.publish("honsegard/led", "lukket");
    client.publish("honsegard/viser", "600");
    client.publish("honsegard/bryter_lukket", "true");
...
void sjekk_taster()
{
  if  ((digitalRead(knapp_apne) == LOW) &&  (digitalRead(knapp_stopp) == LOW)) {
    client.publish("honsegard/status", "sjekk - aktiver");
    aktiver();
  }
  if  ((digitalRead(knapp_lukk) == LOW) &&  (digitalRead(knapp_stopp) == LOW))  {
    client.publish("honsegard/status", "sjekk - deaktiver");
    deaktiver();
  }
  if  (digitalRead(knapp_apne) == LOW) {
    client.publish("honsegard/status", "sjekk - åpner");
    apne("opening");
  }
  if  (digitalRead(knapp_lukk) == LOW)  {
    client.publish("honsegard/status", "sjekk - lukker ");
    lukk("closing");
  }
  if  (digitalRead(knapp_stopp) == LOW) {
    client.publish("honsegard/status", "sjekk - stopp ");
    stopp();
  }
...
{
  status_var = "Aktivert";
  client.publish("honsegard/status", "Aktivert");
  feil = false;
  client.publish("honsegard/status", "Velg Åpne eller Steng");
  display.clearDisplay();
  // text display tests
  display.setTextSize(2);
  display.setTextColor(WHITE);
...
void stopp()
{
  ...
  status_var = "Stoppet";
  // feil=true;
  client.publish("honsegard/status", "Stoppet");
  display.clearDisplay();
  display.setTextSize(3);
  display.setTextColor(WHITE);
...
void deaktiver()
{
  client.publish("honsegard/status", "Utkoblet");
  feil = true;
  display.clearDisplay();
  display.setTextSize(2);
  //    display.setTextColor(WHITE);
...
void        feilrutine(String melding)
{
  ...
  client.publish("honsegard/status", payl);
  feil = true;
  display.clearDisplay();
  display.setTextSize(3);
  //    display.setTextColor(WHITE);
...
void status()
{
  ...
  client.publish("honsegard/status", "deactivate");
  display.display();
}
